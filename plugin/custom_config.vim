...
if $PROJECTS_DIR == ''
  let $PROJECTS_DIR = expand('~/src/')
endif

function! s:HComplete(A,L,P)
  let match = split(glob($PROJECTS_DIR.'/'.a:A.'*'),"\n")
  return map(match,'v:val[strlen($PROJECTS_DIR)+1 : -1]')
endfunction
command! -bar -nargs=1 Hcommand :command! -bar -bang -nargs=1 -complete=customlist,s:HComplete H<args> :<args><lt>bang> $PROJECTS_DIR/<lt>args>
